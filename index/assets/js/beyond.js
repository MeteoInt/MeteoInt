...
function InitiateSideMenu() {
	...
	$("#refresh-toggler").on("click", function() {
		var src = $("#external-frame").attr("src");
		$("#external-frame").attr("src",src);
		return false;
	});
	$("#mdsdiv").on("click",function(){
		var lis = $("#mark_div").find("li");
		if(lis && lis.length>0){
			$("#mark_div").toggle();
		}
	});
	...
	$("#sidebar-collapse").on("click", function() {
		/*if($("#sidebar").is(":visible") || $("#sidebar").toggleClass("hide")){
			var mergin_left = $(".page-breadcrumbs").css("marginLeft");
			if(mergin_left=="4px"){
				$(".page-breadcrumbs").css("marginLeft","0px");
			}else{
				$(".page-breadcrumbs").css("marginLeft","4px");
			}
		}*/
	...
	$(".sidebar-menu").on("click", function(t) {
		...
		if (i && i.length != 0) {
			if (!i.hasClass("menu-dropdown")){
				...
				var li = i.parent();
				li.addClass("active");
				var link_id = li.attr("linkid");
				var menus = mainPage.menus;//获取导航
				...
				for(var i=0;i<len;i++){
					...
					var m_id = menu.id;
					if(m_id==link_id){
						str = "<li class='active'><i class='fa fa-home'></i>" + menu.text + "</li>";
						$("#bread_crumb").html(str);
						makeDownStr(menu.mds);
						return false;
					}else{
						var submenus = menu.submenus;
						if(submenus && submenus.length>0){
							var sub_len = submenus.length;
							for(var j=0;j<sub_len;j++){
								var submenu = submenus[j];
								var sub_id = submenu.id;
								if(sub_id==link_id){
									str = "<li><i class='fa fa-home'></i><a href='#'>" + menu.text + "</a></li>";
									str = str + "<li class='active'>" + submenu.text + "</li>";
									$("#bread_crumb").html(str);
									makeDownStr(submenu.mds);
									return false;
								}else{
									var ssubmenus = submenu.submenus;
									if(ssubmenus && ssubmenus.length>0){
										var ssub_len = ssubmenus.length;
										for(var k=0;k<ssub_len;k++){
											var ssubmenu = ssubmenus[k];
											var ssub_id = ssubmenu.id;
											if(ssub_id==link_id){
												str = "<li><i class='fa fa-home'></i><a href='#'>" + menu.text + "</a></li>";
												str = str + "<li><a href='#'>" + submenu.text + "</a></li>";
												str = str + "<li class='active'>" + ssubmenu.text + "</li>";
												$("#bread_crumb").html(str);
												makeDownStr(ssubmenu.mds);
												return false;
											}
										}
									}
...
function makeDownStr(mds){
	...
	if(mds && mds.length>0){
		...
		for(var i=0;i<len;i++){
			var md = mds[i];
			str = str + '<li class="list-group-item" link="'+md.link+'" onclick="openMd(this)">'+md.text+'</li>';
		}
		str = str + '</ul>';
	}
	$("#mark_div").html(str);
